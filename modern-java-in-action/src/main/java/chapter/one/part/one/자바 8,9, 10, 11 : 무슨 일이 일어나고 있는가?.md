...
- 1챕터에서는 자바가 인기를 얻은 이유와 끊임없이 변화하는 이유에 대해 설명한다.
...
```
## 1.4 스트림
- 스트림 API는 컬렉션 API(외부 반복)와 다르게 데이터 처리를 할 수 있다.
- 스트림은 내부 반복으로 내부에서 모든 데이터가 처리된다.
### 1.4.1 멀티 스레딩은 어렵다
- Java 8 이전엔 스레드 API를 이용해 멀티 스레딩을 하는 것은 어려운 일이었다.
- 또한, 순차적으로 처리해야하는 데이터는 정확성이 불안했다.
- 그러나 스트림 API에선 데이터 정확성을 해결한 멀티 스레딩 방식을 제공한다.
- 방식은 첫번쨰로 각 CPU별로 앞, 중간, 뒤 등 처리 부분을 분리한다. 이것을 포크라고 하고
- 두번쨰는 각 CPU별로 기능을 수행한다.
- 세번쨰는 하나의 CPU가 각 CPU에서 처리된 데이터를 수집하여 합친다.
## 1.5 디폴트 메서드와 자바 모듈
- 기존에 사용하던 인터페이스에 메소드를 추가해야할 경우 인터페이스 뿐 아니라 해당 인터페이스를 구현하는 구현체도
수정이 필요하여 작업 시간이 많이 소요된다.
- 이를 해결하기 위해 default method가 등장하였다. default method를 인터페이스에서 구현했을 때 구현체에서
